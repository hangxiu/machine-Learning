...
def multilayer_perceptron(__X_input_t):
    __hidden_layer1 = tf.nn.sigmoid(tf.add(tf.matmul(__X_input, __weights['w_hidden_layer1']),__biases['b_hidden_layer1']))
    __hidden_layer2 = tf.nn.sigmoid(tf.add(tf.matmul(__hidden_layer1, __weights['w_hidden_layer2']),__biases['b_hidden_layer2']))
    __out_layer = tf.nn.sigmoid(tf.add(tf.matmul(__hidden_layer2, __weights['w_out']),__biases['b_out']))
    return __out_layer
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(__training_epochs):
            ...
            for i in range(__total_batch):
                ...
                __nouse, __loss_t = sess.run([__train_op, __loss_cross_entropy],
...
print("Accuracy:", sess.run(__accuracy_rate,
