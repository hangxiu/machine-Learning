...
# 使用交叉熵作为损失函数
__cross_entropy = tf.reduce_mean(-tf.reduce_sum(__train_y_true *
                                                tf.log(__output_y)))
# region 训练
# 使用梯度下降法优化模型，学习率为0.01。
# 训练周期为1000，每个训练周期使用60个样本
__train_op = tf.train.GradientDescentOptimizer(0.01).minimize(__cross_entropy)
for i in range(1000):
    __batch = __mnist.train.next_batch(60)
    __session.run(__train_op,
                  feed_dict={
                      __input_x: __batch[0],
                      __train_y_true: __batch[1]
                  })
...
__perdict_num_list = __session.run(tf.argmax(__output_y, 1),
                                   feed_dict={__input_x: __test_x})
__accuracy = 0
