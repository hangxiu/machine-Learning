...
def max_pooling_2x2(__max_pooling_target):
    return tf.nn.max_pool(__max_pooling_target,
                          ksize=[1, 2, 2, 1],
                          strides=[1, 2, 2, 1],
                          padding='SAME')
...
__weights = {
    ...
    '__w_conv1':
    tf.Variable(tf.truncated_normal(shape=[5, 5, 1, 32], stddev=0.1)),
    # [filter_height, filter_width, in_channels, out_channels]
    # 5x5x32x64,同样是卷积核的视野大小是5x5，因为上一层使用了32个filter，
    # 所以上层生成结果的通道为32，该层卷积核有64个filter
    '__w_conv2':
    tf.Variable(tf.truncated_normal(shape=[5, 5, 32, 64], stddev=0.1)),
    '__w_fc1':
    tf.Variable(tf.truncated_normal(shape=[4 * 4 * 64, 1024], stddev=0.1)),
    '__w_fc2':
    tf.Variable(tf.truncated_normal(shape=[1024, 10], stddev=0.1)),
}
__biases = {
...
def conv_net(__x_t, __keep_probability_t):
    ...
    __h_conv1 = tf.nn.relu(
        conv2d(__x_image, __weights['__w_conv1']) + __biases['__b_conv1'])
    print(np.shape(__h_conv1))
    __h_pool1 = max_pooling_2x2(__h_conv1)
    print(np.shape(__h_pool1))
    __h_conv2 = tf.nn.relu(
        conv2d(__h_pool1, __weights['__w_conv2']) + __biases['__b_conv2'])
    ...
    __h_fc1 = tf.nn.relu(
        tf.matmul(__h_pool2_flat, __weights['__w_fc1']) + __biases['__b_fc1'])
    ...
    # 返回 Tensor
    __y_conv_logits = tf.matmul(__h_fc1_drop,
                                __weights['__w_fc2']) + __biases['__b_fc2']
...
__train_op = tf.train.AdamOptimizer(__learning_rate).minimize(
    __loss_cross_entropy)

__accuracy = tf.reduce_mean(
    tf.cast(tf.equal(tf.argmax(__out_softmax, 1), tf.argmax(__Y_true, 1)),
            tf.float32))
...
with tf.Session() as sess:
    ...
    for i in range(__training_epochs):
        ...
        if i % __display_step == 0:
            __train_accuracy = sess.run(__accuracy,
                                        feed_dict={
                                            __X_input: __x_batch,
                                            __Y_true: __y_batch,
        ...
        # sess.run(__train_op, feed_dict={__X_input: __x_batch,
        #          __Y_true: __y_batch,
        #          __keep_probability: 0.5})
        __train_op.run(feed_dict={
            __X_input: __x_batch,
            __Y_true: __y_batch,
            __keep_probability: 0.5
        })
