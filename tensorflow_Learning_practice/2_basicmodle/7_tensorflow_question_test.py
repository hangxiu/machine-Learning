...
def mat_mul():
    A = tf.random.normal([2, 1, 4, 5])
    B = tf.random.normal([2, 3, 1, 5])
...
def while_loop():
    ...
    return tf.while_loop(
        lambda n, i, other_param: i < n, lambda n, i, other_param: (
            n, i + 1, other_param + '???'), (tf.shape(A)[2], 0, ''))


def condd():
    A = tf.random_normal([2, 3, 10])
    return tf.cond(tf.equal(tf.shape(A)[2],
                            10), lambda: 'yesyes', lambda: 'nono')


def tf_scope_test():
    with tf.name_scope('namescope'):
        var_1 = tf.Variable(initial_value=[0], name='var_1')
        var_2 = tf.get_variable(name='var_2', shape=[
            1,
        ])
    with tf.variable_scope('variable_scope'):
        var_3 = tf.Variable(initial_value=[0], name='var_3')
        var_4 = tf.get_variable(name='var_4', shape=[
            1,
        ])
...
def tf_add_node_mem_test():  # 新建op节点
    ...
    w = tf.get_variable("test", [1], initializer=ini, dtype=tf.int32)
    i = 0
    time1 = time.time()
...
def concat():
    ...
    return tf.shape(tf.concat([A, B], axis=0))
